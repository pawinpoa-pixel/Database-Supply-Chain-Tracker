...
def create_purchase_order(
    body: schemas.PurchaseOrderCreate, db: Session = Depends(get_db), _=Depends(get_current_user)
):
...
def delete_purchase_order(po_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    po = _get_po_or_404(db, po_id)
...
def remove_purchase_order_item(
    po_id: int, item_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)
):
    po = _get_po_or_404(db, po_id)
    if po.status != "pending":
